...
OUTPUT_DIR = "figures"
RESULT_XLSX = "analysis_results_cv.xlsx"
...
# =========================
# 7. Train 5-Fold Cross-Validation Models
# =========================

def fit_cv_logistic_model(X, y, label_name):
    """
    Fit a logistic regression model using 5-Fold CV and return average coefficients & std.
    """
    if y.nunique() < 2:
        print(f"\n===== {label_name} =====")
...
coef_vitro = fit_cv_logistic_model(X, y_vitro, "In vitro model")
coef_vivo = fit_cv_logistic_model(X, y_vivo, "In vivo model")

# =========================
# 8. Compare feature effects between in vitro and in vivo
# =========================

coef_compare = coef_vitro[["feature", "coefficient", "std_dev"]].rename(
    columns={"coefficient": "coef_in_vitro", "std_dev": "std_in_vitro"}
).merge(
    coef_vivo[["feature", "coefficient", "std_dev"]].rename(
        columns={"coefficient": "coef_in_vivo", "std_dev": "std_in_vivo"}
    ),
    on="feature",
    how="outer"
)
...
# Calculate overall stability for coloring in Figure 2
coef_compare["total_std"] = coef_compare["std_in_vitro"].fillna(0) + coef_compare["std_in_vivo"].fillna(0)
max_std = coef_compare["total_std"].max()
if max_std > 0:
    coef_compare["stability_weight"] = 1 - (coef_compare["total_std"] / max_std)
else:
    coef_compare["stability_weight"] = 1.0

coef_compare = coef_compare.sort_values("coef_diff", key=np.abs, ascending=False)

print("\n===== Feature effect comparison =====")
print(coef_compare[["feature", "coef_in_vitro", "coef_in_vivo", "total_std"]].head(20))
...
# ========================================================
# 12. Figure 2: Coefficient comparison with CV error bars
# ========================================================

fig2, ax = plt.subplots(figsize=(16, 14)) 

ax.axhline(0, linestyle="--", linewidth=1, color='gray', alpha=0.4)
ax.axvline(0, linestyle="--", linewidth=1, color='gray', alpha=0.4)
...
highlighted_features = labeled_df["feature"].values

for idx, row in plot_coef_compare.iterrows():
    base_color = get_color(row["feature"])
    is_highlighted = row["feature"] in highlighted_features
    
    ax.errorbar(
        row["coef_in_vitro"], row["coef_in_vivo"],
        xerr=row["std_in_vitro"], yerr=row["std_in_vivo"],
        fmt='none',
        ecolor='gray',
        elinewidth=1.5 if is_highlighted else 0.5, 
        alpha=0.5 if is_highlighted else 0.1,  
        zorder=1
    )
    
    if is_highlighted:
        ax.scatter(row["coef_in_vitro"], row["coef_in_vivo"], 
                   color=base_color, alpha=0.9, s=180, edgecolors='white', linewidth=1.5, zorder=5)
    else:
        ax.scatter(row["coef_in_vitro"], row["coef_in_vivo"], 
                   color=base_color, alpha=0.2, s=30, zorder=2)
...
legend_elements = [
    ...
    Patch(facecolor='lightgray', label='Error Bars = Standard Deviation (Volatility)'),
...
for i, (idx, row) in enumerate(labeled_df.iterrows()):
    ...
    offset_y = 0.025 if point_num % 2 == 0 else -0.025
    ax.text(
        x_val, y_val + offset_y, str(point_num),
...
ax.set_xlabel("Mean Coefficient in In Vitro Model", fontsize=14, fontweight='bold')
ax.set_ylabel("Mean Coefficient in In Vivo Model", fontsize=14, fontweight='bold')
ax.set_title("Feature Effects Comparison with 5-Fold Stability", fontsize=16, fontweight='bold', pad=25)

ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1.02, 1), 
          fontsize=11, frameon=True, ncol=1, title="Feature Index Map", 
          title_fontsize=13, labelspacing=1.2)

plt.tight_layout()
fig2.savefig(os.path.join(OUTPUT_DIR, "figure_2_final_cv_errorbars.png"), bbox_inches="tight", dpi=300)
plt.close(fig2)
# =========================
# 13. Figure 3: Modification enrichment
# =========================
...
# =========================
# 14. Figure 4-5: Top coefficient bar plots (With Error Bars)
# =========================

def plot_top_coefficients(coef_df, title, save_path, top_n=12):
    if coef_df.empty or coef_df["coefficient"].isna().all(): return
    plot_df = coef_df.head(top_n).copy().sort_values("coefficient")
    fig = plt.figure(figsize=(8, 5))
    
    plt.barh(plot_df["feature"], plot_df["coefficient"], xerr=plot_df["std_dev"], 
             capsize=4, color="#5B9BD5", edgecolor="black")
             
    plt.xlabel("Mean Coefficient (± Std Dev)")
    plt.title(title)
    plt.tight_layout()
    fig.savefig(save_path, bbox_inches="tight")
    plt.close(fig)

plot_top_coefficients(coef_vitro, "In vitro Top Features (5-Fold Mean)", os.path.join(OUTPUT_DIR, "figure_4_vitro_coef_cv.png"))
plot_top_coefficients(coef_vivo, "In vivo Top Features (5-Fold Mean)", os.path.join(OUTPUT_DIR, "figure_5_vivo_coef_cv.png"))
...
with pd.ExcelWriter(RESULT_XLSX, engine="openpyxl") as writer:
    ...
    if not coef_vitro.empty:
        coef_vitro.to_excel(writer, sheet_name="coef_in_vitro_cv", index=False)
    if not coef_vivo.empty:
        coef_vivo.to_excel(writer, sheet_name="coef_in_vivo_cv", index=False)
    coef_compare.to_excel(writer, sheet_name="coef_comparison_cv", index=False)
...
print("\nDone. All CV analysis results and figures saved.")
